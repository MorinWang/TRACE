        ...
        try:
            ...
            for qa_idx, qa in enumerate(sample["qa"]):
                try:
                    ...
                    logger.error("Sample %d QA %d: missing key %s — qa=%s", sample_idx, qa_idx, e, qa)
                    raise e
                except Exception as e:
                    logger.error("Sample %d QA %d: %s — qa=%s", sample_idx, qa_idx, e, qa)
                    raise e
            ...
            # Per-sample stats logged at debug level (silent by default)
            logger.debug("Sample %d: %d QAs (%d with image evidence)",
                         sample_idx, sample_qa_count, sample_image_qa_count)

        except Exception as e:
            logger.error("Error processing sample %d: %s", sample_idx, e)
            raise e

    # Overall stats: single info line instead of 5 prints
    logger.info(
        "Loaded %d samples, %d total QAs (avg %.1f, min %d, max %d per sample); %d with image evidence",
        len(samples), total_qa, total_qa / len(samples),
        min(qa_counts_per_sample), max(qa_counts_per_sample), total_image_qa,
    )
