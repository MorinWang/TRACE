"""
Robust A-MEM memory layer.

Key differences from the upstream A-Mem reference implementation:
  - No response_format / JSON schema dependency in LLM calls
  - Plain-text prompts with section-marker parsing (via trace.llm_text_io)
...
from trace.embedding_retriever import SimpleEmbeddingRetriever, simple_tokenize
...
class RobustLLMController:
    """Factory for the robust OpenAI-compatible LLM controller.

    The release pipeline targets OpenRouter (OpenAI-compatible) only; the
    backend argument is kept for upstream-compatibility but ``"openai"`` is
    ...
    def __init__(self,
        ...
        if backend != "openai":
            raise ValueError(
                f"Backend {backend!r} is not supported in this release; "
                "only 'openai' (OpenAI / OpenRouter / OpenAI-compatible endpoints)."
            )
    ...
    def analyze_content(content: str, llm_controller: RobustLLMController) -> Dict:
        ...
        try:
            ...
            if not analysis["keywords"]:
                ...
                from trace.llm_text_io import _parse_list_items
            ...
            from trace.llm_text_io import _heuristic_keywords, _heuristic_context
            return {
...
class RobustAgenticMemorySystem:
    ...
    def __init__(self,
        ...
        self.llm_controller = RobustLLMController(
            llm_backend, llm_model, api_key, api_base, check_connection,
        )
        self.evo_cnt = 0
    ...
    def add_note(self, content: str, time: str = None, **kwargs) -> str:
        ...
        note = RobustMemoryNote(
            content=content,
            llm_controller=None if self.skip_evolution else self.llm_controller,
            timestamp=time,
            **kwargs,
        )
